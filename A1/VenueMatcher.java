...
public class VenueMatcher {
	...
	private static List<String> oldJobRequests = new ArrayList<>();
...
//     newJobRequests.addAll(existingJobRequests);
		...
		while (true) {
			...
			case 6:
				...
				try {
					...
					fileHandler.writeJobRequestsToFile(finalSummaries, "requests.csv");
				} catch (CustomException e) {
					System.out.println("Error: " + e.getMessage());
				}
	...
	// Method to display old job requests (Muen 1)
	private static void displayOldJobRequests() {
	...
	// Method to select venues by category (Menu 2)
	...
	// Method to get the selected category (Menu 3)
	...
	// Method to display venues for a selected category (Menu 4)
	...
	// Method to select a venue from the displayed venues (Menu 3.1)
	...
	// Method to prompt the user for further action after selecting a venue (Memu
	// 3.2)
	...
	private static void showHiringMenu(Venue venue) {
		...
		if (confirmOrder.equalsIgnoreCase("y")) {
			...
			saveHiringDetails(venue, numberOfHours, date, time, eventName, artistName, requesterName, eventType);
	...
	private static void saveHiringDetails(Venue venue, int numberOfHours, String date, String time, String eventName,
		...
		String orderSummary = String.format(
				...
				requesterName, venue.getName(), eventName, artistName, date, time, numberOfHours, numberOfHours * 550,
				numberOfHours * 550 * 0.1);

		orderSummaries.add(orderSummary);

		//		String finalSummary = String.format("Client: %s\nVenue: %s\nEvent: %s\nArtist: %s\nDate: %s\nTime: %s\nHours: %d\n",
//                requesterName, venue.getName(), eventName, artistName, date, time, numberOfHours);
	...
	// Method to display order summary (Menu 5)
	...
	// Method to select a venue from the venue list (MENU 2/3.x)
	...
	// Method to select a venue from the matching list (MENU 2/3.x.x)
